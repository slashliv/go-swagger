...
```sh
go install github.com/slashliv/go-swagger/cmd/swagger@latest
```

You may need to CGO disabled, see [#3171](https://github.com/slashliv/go-swagger/issues/3171).

```sh
CGO_ENABLED=0 go install github.com/slashliv/go-swagger/cmd/swagger@latest
```
...
[![GitHub Downloads (all assets, latest release)](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Frepos%2Fgo-swagger%2Fgo-swagger%2Freleases%2Flatest&label=Latest%20tarball&query=%24.tarball_url)](https://github.com/slashliv/go-swagger/releases/latest)
...
```sh
dir=$(mktemp -d) 
git clone https://github.com/slashliv/go-swagger "$dir" 
cd "$dir"
go install ./cmd/swagger
```
